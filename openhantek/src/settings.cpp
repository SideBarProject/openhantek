...
DsoSettings::DsoSettings(QWidget *parent) : QObject(parent) {
    ...
    this->scope.spectrumWindow = DSOAnalyzer::WindowFunction::WINDOW_HANN;
    ...
    this->view.interpolation = DSO::InterpolationMode::INTERPOLATION_LINEAR;
...
void DsoSettings::setChannelCount(unsigned int channels) {
    qDebug() << "DsoSettings::setChannelCount: channels = " << channels << "scope.voltage.count()" << scope.voltage.count() << endl;
	...
	for(int channel = 0; channel < (int) channels; ++channel) {
		...
		if(this->scope.voltage.count() <= channel + 1) {
            DSO::dsoSettingsChannel newVoltage;
//            DsoSettingsScopeVoltage newVoltage;
			newVoltage.gain = 1.0;
            newVoltage.misc = DSO::Coupling::COUPLING_DC;
		...
		// View
        // Colorsthis->settings->scope.physicalChannels)
		// Screen
...
//		DsoSettingsScopeVoltage newVoltage;
		newVoltage.gain = 1.0;
        newVoltage.misc = (int)DSOAnalyzer::MathMode::ADD_CH1_CH2;
    ...
    qDebug() << "DsoSettings::setChannelCount: scope.voltage.count() after channel insertion: " << scope.voltage.count() << endl;
...
int DsoSettings::load(const QString &fileName) {
    ...
    if(fileName.isEmpty()) {
        qDebug() << "DsoSettings::load empty filename" << endl;
	...
	if(settingsLoader->contains("spectrumWindow"))
        this->scope.spectrumWindow = (DSOAnalyzer::WindowFunction) settingsLoader->value("spectrumWindow").toInt();
	settingsLoader->endGroup();
...
/// \param fileName Optional filename to write the settings to an ini file.
/// \return 0 on success, negative on error.
int DsoSettings::save(const QString &fileName) {
    ...
    if(complete) {
        ...
        qDebug() << "writing settings to " << settingsSaver->fileName() << endl;
    }
    else
    ...
    settingsSaver->setValue("mode", (QVariant&)this->scope.trigger.mode);
