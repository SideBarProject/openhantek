...
DsoWidget::DsoWidget(CurrentDevice *currentDevice, DsoSettings *settings, QWidget *parent, Qt::WindowFlags flags) : QWidget(parent, flags) {

    this->settings = settings;
    this->m_currentDevice = currentDevice;
    qDebug() << "DsoWidget: settings" << this->settings->scope.horizontal.format << endl;
    ...
    this->m_currentDevice->setGlGenerator(this->generator);
    qDebug() << "connecting singleShotFinish to stopped" << endl;

    connect(this->generator, SIGNAL(singleShotFinish()), parent, SLOT(stopped()) );
...
void DsoWidget::updateTriggerDetails() {
    ...
    this->settingsTriggerLabel->setText(tr("%1  %2  %3  %4").arg(QString::fromStdString(this->settings->scope.voltage[this->settings->scope.trigger.source].name),
                                        Dso::slopeString(this->settings->scope.trigger.slope), levelString, pretriggerString));
...
void DsoWidget::updateVoltageDetails(unsigned int channel) {
	...
	if(this->settings->scope.voltage[channel].used)
        this->measurementGainLabel[channel]->setText(Helper::valueToString(this->settings->scope.voltage[channel].gain/DIVS_VOLTAGE, Helper::UNIT_VOLTS, 3) + tr("/div"));
	else
		this->measurementGainLabel[channel]->setText(QString());
}
...
void DsoWidget::updateVoltageGain(unsigned int channel) {
     qDebug() << "DsoWidget::updateVoltageGain on channel "<< channel << endl;
     if(channel >= (unsigned int) this->settings->scope.voltage.count())
		return;
...
bool DsoWidget::exportAs() {
	...
	if(fileDialog.exec() != QDialog::Accepted)
		return false;
/*
    Exporter exporter(this->settings, static_cast<QWidget *>(this->parent()));
