            ...
            case DSO::GraphFormat::GRAPHFORMAT_TY:
                ...
                for(int mode = DSO::ChannelMode::CHANNELMODE_VOLTAGE; mode < DSO::ChannelMode::CHANNELMODE_COUNT; ++mode) {
					for(int channel = 0; channel < this->settings->scope.voltage.count(); ++channel) {
                        if((mode == DSO::ChannelMode::CHANNELMODE_VOLTAGE) ? this->settings->scope.voltage[channel].used : this->settings->scope.spectrum[channel].used) {
							...
							for(int index = this->generator->digitalPhosphorDepth - 1; index >= 0; index--) {
								if(this->generator->vaChannel[mode][channel][index]->data) {
                                    if(mode == DSO::ChannelMode::CHANNELMODE_VOLTAGE)
                                    ...
                                    glDrawArrays((this->settings->view.interpolation == DSO::InterpolationMode::INTERPOLATION_OFF) ? GL_POINTS : GL_LINE_STRIP, 0, this->generator->vaChannel[mode][channel][index]->getSize() / 2);
								}
							}
						}
					}
            ...
            case DSO::GraphFormat::GRAPHFORMAT_XY:
				...
				for(int channel = 0; channel < this->settings->scope.voltage.count() - 1; channel += 2) {
					if(this->settings->scope.voltage[channel].used) {
						// Draw graph for all available depths
						for(int index = this->generator->digitalPhosphorDepth - 1; index >= 0; index--) {
                            if(this->generator->vaChannel[DSO::ChannelMode::CHANNELMODE_VOLTAGE][channel][index]->data) {
								this->qglColor(this->settings->view.color.screen.voltage[channel].darker(fadingFactor[index]));
                                glVertexPointer(2, GL_FLOAT, 0, this->generator->vaChannel[DSO::ChannelMode::CHANNELMODE_VOLTAGE][channel][index]->data);
                                glDrawArrays((this->settings->view.interpolation == DSO::InterpolationMode::INTERPOLATION_OFF) ? GL_POINTS : GL_LINE_STRIP, 0,
                                             this->generator->vaChannel[DSO::ChannelMode::CHANNELMODE_VOLTAGE][channel][index]->getSize() / 2);
							}
						}
					}
				}
